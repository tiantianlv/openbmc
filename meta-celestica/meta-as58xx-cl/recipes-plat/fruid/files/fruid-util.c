...
static int get_fruid_status(char* name)
{
	int ret;
	FILE *fp;
	int error;
	...
	if(strcmp(name, "PSU 1-1") == 0)
		memcpy(file, "/tmp/PSU1", strlen("/tmp/PSU1"));
	else if(strcmp(name, "PSU 1-2") == 0)
		memcpy(file, "/tmp/PSU2", strlen("/tmp/PSU2"));
	else if(strcmp(name, "PSU 2-1") == 0)
		memcpy(file, "/tmp/PSU3", strlen("/tmp/PSU3"));
	else if(strcmp(name, "PSU 2-2") == 0)
		memcpy(file, "/tmp/PSU4", strlen("/tmp/PSU4"));
	else
...
static int set_fruid_status(char* name, int val)
{
	int ret;
	FILE *fp;
	char tmp[10];
...
void get_fruid_info(uint8_t fru, char *path, char* name) {
  ...
  error = get_fruid_status(name);
  if(error >= 3) {
    fprintf(stderr, "Read %s remains 3 errors!\n", name);
  	return ;
  }
  ...
  if (ret) {
    fprintf(stderr, "Failed print FRUID for %s\nCheck syslog for errors!\n",
  	...
  	if(fruid.chassis.flag == 0 && fruid.board.flag == 0 && fruid.product.flag == 0) {
		error = get_fruid_status(name);
		set_fruid_status(name, ++error);
  	} else {
        set_fruid_status(name, 0);
    }
